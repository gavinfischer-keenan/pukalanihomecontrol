...
const DEFAULT_LAYERS = {
  ...
  radar:         { label: "🌧️ Radar",               live: true,  enabled: true  },
...
function App() {
  ...
  // Nearby AISHub vessels — 30s poll (in-memory cache, no DB)
  useEffect(() => {
    const go = async () => {
      try {
        const data = await fetch(`${API_BASE}/api/vessels/nearby`).then(r => r.json());
        setNearbyVessels(Array.isArray(data) ? data : []);
      } catch { setNearbyVessels([]); }
    };
    go();
  ...
  // Handle clicks from Frequent Visitors sidebar
  const handleFrequentSelect = useCallback((entity) => {
    setSelectedFrequentEntity(entity);
    const id = entity.identifier;
    if (entity.entity_type === 'vessel') {
      const live = vessels.find(v => String(v.entity_id) === String(id));
      if (live) {
        setSelected({ ...live, _type: 'vessel' });
      } else {
        // Not live — open detail panel with known-entities data
        setSelected({
          entity_id: id,
          vessel_name: entity.friendly_name || entity.name || null,
          vessel_type: entity.vessel_type || null,
          _type: 'vessel',
        });
      }
    } else if (entity.entity_type === 'aircraft') {
      const live = aircraft.find(a => a.hex === id);
      if (live) {
        setSelected({ ...live, _type: 'aircraft' });
      } else {
        setSelected({
          hex: id,
          flight: entity.friendly_name || entity.name || null,
          _type: 'aircraft',
        });
      }
    }
  ...
  return (
    <div className="app-root">
      ...
      {!inIframe && <FrequentVisitorsSidebar
        apiBase={API_BASE}
        visible={showFrequentSidebar}
        onClose={() => setShowFrequentSidebar(false)}
        onSelectEntity={handleFrequentSelect}
        liveVessels={vessels}
      />}
      ...
      {!inIframe && <StatusBar aircraft={aircraft} vessels={vessels} lastUpdate={lastUpdate} status={status} />}
    </div>
  );
}
