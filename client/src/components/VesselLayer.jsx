...
export function classifyVessel(typeCode, mmsi) {
  // MMSI-based lookup first (works even with null vessel_type)
...
export default function VesselLayer({ vessels, selected, showLabels, onSelect, predictions }) {
  ...
  useEffect(() => {
    // We must capture the refs because React may clear the ref objects on unmount before this runs
    ...
    return () => {
      ...
      // Ensure we clear the refs so a remount starts fresh
      markersRef.current = {};
      labelsRef.current = {};
    };
  }, [map]);
