...
|---|---|---|---|---|---|---|
| Aqara Cam 1 (Front Garden from Roof) | `aqara_cam_1` | 192.168.1.213 | 8554 | `rtsp://772:885@192.168.1.213:8554/1080p` | `rtsp://772:885@192.168.1.213:8554/1520p` | ✅ Active |
| Aqara Cam 2 (Back Deck) | `aqara_cam_2` | 192.168.1.143 | 8554 | `rtsp://772:885@192.168.1.143:8554/1080p` | `rtsp://772:885@192.168.1.143:8554/1520p` | ✅ Active |
...
- **Aqara Cameras**: IP-based RTSP streaming (192.168.1.213:8554, 192.168.1.143:8554). Audio split to BirdNET, Video split to Frigate NVR.
...
1. **Camera Audio Pipeline**: Aqara RTSP streams (`rtsp://...192.168.1.213:8554/1080p` and `rtsp://...192.168.1.143:8554/1080p`) → FFmpeg audio decode → BirdNET Go (CT112) → Acoustic species detection → SQLite DB & SSE real-time stream.
...
- **CT114 Services** (192.168.1.114):
  - `photo-chrono-ml` (port 7778, Photo Chronologizer ML API ??? stateless face detection via InsightFace/ONNX)
  - `display-server` (port 3000, HDMI kiosk projection)
  - `roofcam` (port 7780, Roofcam Web Server)
  - `nrsc5-engine` (port 3011, NRSC5 Engine / HD Radio)
  - `utilities` (port 3114, Utilities API ??? PDF Maker/Shrinker)
  - **DECOMMISSIONED**: `photo-chrono` on port 7777 has been removed from CT114. The full Photo Chronologizer app now runs on Gavin's Windows desktop (localhost:7777). CT114 only hosts the stateless ML API on port 7778.
...
- **CT114**: display-server, utilities, photo-chrono-ml, nrsc5-engine, roofcam
...
- `rtl-tcp-ais.service` on Proxmox host (`/etc/systemd/system/rtl-tcp-ais.service`) now runs with `Restart=always` (was `Restart=no`, which caused prolonged AIS outages after USB disruptions).
- AIS-catcher on CT106 connects to rtl_tcp at `192.168.1.100:1234` for raw SDR samples.
...
### Photo Chronologizer Desktop (Windows)

The full Photo Chronologizer application now runs on Gavin's Windows desktop machine, with ML offloaded to CT114.

- **URL**: `localhost:7777`
- **Backend**: FastAPI + SQLite (`C:\Users\gavin\Downloads\photo-chrono-desktop`)
- **Frontend**: React/Vite (pre-built static bundle, served by FastAPI)
- **ML API**: Face detection calls to `CT114:7778` (InsightFace/ONNX, stateless). Photos never leave the local machine ??? only face crops are sent to the ML API for inference.
- **Architecture**: Desktop app handles all photo management, timeline ordering, and session state locally. The ML API on CT114 is a pure inference endpoint (no storage, no state).
